    ...
    const updatedAchievements = achievementDefinitions.map((def) => {
      ...
      switch (def.category) {
        ...
        case "tools":
          ...
          currentProgress = Math.min(def.requirement, Math.floor(userStats.totalMoodLogs / 3))
      ...
      const justUnlocked = isUnlocked && !wasUnlocked
    ...
    <>
      <Card>
        <CardHeader>
          ...
          <CardDescription>Track your progress and unlock badges for your mental health journey</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          {unlockedAchievements.length > 0 && (
            <div>
              <h3 className="font-semibold mb-3 text-green-700">Unlocked Achievements</h3>
          ...
          {inProgressAchievements.length > 0 && (
            <div>
              <h3 className="font-semibold mb-3 text-orange-700">In Progress</h3>
              <div className="grid gap-3">
          ...
          {lockedAchievements.length > 0 && (
            <div>
              <h3 className="font-semibold mb-3 text-muted-foreground">Locked</h3>
              <div className="grid gap-3">
                {lockedAchievements.slice(0, 3).map((achievement) => {
                  ...
                  return (
                    <div
                      key={achievement.id}
                      className="flex items-center space-x-3 p-3 bg-muted/30 rounded-lg opacity-60"
                    >
                      <div className="bg-muted p-2 rounded-full">
                        <IconComponent className="h-5 w-5 text-muted-foreground" />
                      </div>
      ...
      <Dialog open={showCelebration} onOpenChange={setShowCelebration}>
        <DialogContent className="text-center">
            ...
            <DialogDescription className="text-lg">
              {newAchievement && (
                <div className="space-y-2">
                  <div className="font-semibold text-foreground">{newAchievement.title}</div>
          ...
          <div className="pt-4">
            <Button onClick={() => setShowCelebration(false)} className="bg-yellow-500 hover:bg-yellow-600">
              Awesome!
            </Button>
          </div>
        </DialogContent>
