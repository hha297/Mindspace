    ...
    <div className="space-y-6">
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Express your thoughts and feelings through writing. Choose a prompt or start with free writing.
          </CardDescription>
        </CardHeader>
          ...
          {!selectedPrompt && (
            <div className="space-y-4">
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold">Choose a writing prompt</h3>
                <Button onClick={startFreeWrite} variant="outline" size="sm">
                  Free Write
                </Button>
              </div>
                    ...
                    >
                      <div className="flex items-start space-x-3">
                        ...
                        <div className="flex-1">
                          <Badge className={`${getCategoryColor(prompt.category)} mb-2`} variant="secondary">
                            {prompt.category}
                          </Badge>
          ...
          {(selectedPrompt || journalEntry) && (
            <div className="space-y-4">
              {selectedPrompt && (
                <div className="p-4 bg-muted/50 rounded-lg">
                  <div className="flex items-center justify-between mb-2">
                    <Badge className={getCategoryColor(selectedPrompt.category)} variant="secondary">
                      {selectedPrompt.category}
                    </Badge>
                    <Button onClick={() => setSelectedPrompt(null)} variant="ghost" size="sm">
                      Change Prompt
                    </Button>
                  </div>
                  <p className="text-sm font-medium">{selectedPrompt.prompt}</p>
                </div>
              )}
              ...
              <div className="flex justify-end space-x-2">
                ...
                <Button onClick={saveEntry} disabled={isSaving || !journalEntry.trim()}>
                  {isSaving ? (
                    "Saving..."
                  ) : (
                    <>
            ...
            <div className="space-y-4">
              {savedEntries.slice(0, 3).map((entry) => (
                <div key={entry.id} className="p-4 border rounded-lg">
                  ...
                  {entry.prompt && <p className="text-sm text-muted-foreground mb-2 italic">"{entry.prompt}"</p>}
