    ...
    ranges: [
      {
        ...
        max: 7,
        level: "Low Stress",
        description: "You're managing stress well. Keep up the good work with your current coping strategies.",
...
export function SelfAssessmentQuiz() {
  ...
  const completeQuiz = () => {
    ...
    const scoreRange = currentQuiz.scoring.ranges.find((range) => totalScore >= range.min && totalScore <= range.max)
        ...
        <CardContent className="space-y-6">
          <div className={`p-6 rounded-lg border ${result.color}`}>
            <div className="flex items-center space-x-3 mb-4">
              ...
              <div>
                <h3 className="text-xl font-semibold">{result.level}</h3>
          ...
          <div className="space-y-4">
            ...
            <div className="grid gap-3">
              {result.level === "Low Stress" && (
                <>
                  <div className="p-3 bg-muted/50 rounded-lg">
                    <p className="text-sm">Continue your current stress management practices</p>
                  </div>
                  <div className="p-3 bg-muted/50 rounded-lg">
                    <p className="text-sm">Consider sharing your strategies with friends who might benefit</p>
                  </div>
                </>
              )}
              {result.level === "Moderate Stress" && (
                <>
                  <div className="p-3 bg-muted/50 rounded-lg">
                    <p className="text-sm">Try our breathing exercises or journaling tools</p>
                  </div>
                  <div className="p-3 bg-muted/50 rounded-lg">
                    <p className="text-sm">Consider establishing a regular self-care routine</p>
                  </div>
                  <div className="p-3 bg-muted/50 rounded-lg">
                    <p className="text-sm">Explore our stress management resources</p>
                  </div>
                </>
              )}
              {result.level === "High Stress" && (
                <>
                  <div className="p-3 bg-muted/50 rounded-lg">
                    <p className="text-sm">Consider speaking with a counselor or mental health professional</p>
                  </div>
                  <div className="p-3 bg-muted/50 rounded-lg">
                    <p className="text-sm">Use our crisis resources if you need immediate support</p>
                  </div>
                  <div className="p-3 bg-muted/50 rounded-lg">
                    <p className="text-sm">Try daily stress-reduction activities like breathing exercises</p>
                  </div>
                </>
              )}
            </div>
      ...
      <CardContent className="space-y-6">
        ...
        <div className="flex justify-between">
          ...
          <Button onClick={nextQuestion} disabled={!canProceed}>
            {currentQuestion === currentQuiz.questions.length - 1 ? "Complete" : "Next"}
